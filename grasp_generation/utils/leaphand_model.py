...
class LeapHandModel:
    def __init__(
        ...
        print(f"n_dofs: {self.n_dofs}")
        print(f"self.chain: \n{self.chain}")
        print(f"self.chain._root: \n{self.chain._root}")
        ...
        def build_mesh_recurse(body):
            if len(body.link.visuals) > 0:
                ...
                link_faces = []
                n_link_vertices = 0
                print(f"len_link_visuals: {len(body.link.visuals)}")
                print(f"body.link: {body.link}")
                print(f"body.link.visuals: {body.link.visuals}")
                for visual in body.link.visuals:
                    print(f"visual.geom_type: {visual.geom_type}")
                    ...
                    if visual.geom_type == "box":
                        ...
                        mesh_filename = (
                            ...
                            else visual.geom_param[0]
                        )
                        link_mesh = tm.load_mesh(
                            os.path.join(mesh_path, mesh_filename), process=False
                        )

                        # 如果geom_param是一个列表，并且有更多的元素（即缩放因子），则使用该缩放因子
                        if (
                            isinstance(visual.geom_param, list)
                            and len(visual.geom_param) > 1
                        ):
                            scale = torch.tensor(
        ...
        if self.joints_lower:  # 检查列表是否不为空
            self.joints_lower = torch.stack(self.joints_lower).float().to(device)
        else:
            self.joints_lower = torch.tensor([], dtype=torch.float).to(device)

        if self.joints_upper:  # 检查列表是否不为空
            self.joints_upper = torch.stack(self.joints_upper).float().to(device)
        else:
            self.joints_upper = torch.tensor([], dtype=torch.float).to(device)
        # self.joints_lower = torch.stack(self.joints_lower).float().to(device)
        # self.joints_upper = torch.stack(self.joints_upper).float().to(device)
