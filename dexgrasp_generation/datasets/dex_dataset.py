...
from datasets.leap_hand_builder import LeapHandBuilder
...
def plane2pose(plane_parameters):
    ...
    r2 = np.zeros_like(r3)
    r2[0], r2[1], r2[2] = (
        (-r3[1], r3[0], 0) if r3[2] * r3[2] <= 0.5 else (-r3[2], 0, r3[0])
    )
    r1 = np.cross(r2, r3)
...
class DFCDataset(Dataset):
    def __init__(self, cfg, mode):
        ...
        if cfg["use_Shadow"]:
            self.hand_mesh_dir = pjoin(
                self.root_path, dataset_cfg["shadow_hand_mesh_dir"]
            )
            self.hand_urdf_path = pjoin(self.root_path, dataset_cfg["shadow_urdf_path"])
            self.hand_builder = ShadowHandBuilder(
                self.hand_mesh_dir, self.hand_urdf_path
            )
        elif cfg["use_LEAP"]:
            self.hand_mesh_dir = pjoin(
                self.root_path, dataset_cfg["leap_hand_mesh_dir"]
            )
            self.hand_urdf_path = pjoin(self.root_path, dataset_cfg["leap_urdf_path"])
            print(
                f"self.root_path:{self.root_path} self.hand_mesh_dir:{self.hand_mesh_dir} self.hand_urdf_path:{self.hand_urdf_path}"
            )
            print(f"传递给LeapHandBuilder的urdf_path: {self.hand_urdf_path}")

            self.hand_builder = LeapHandBuilder(self.hand_mesh_dir, self.hand_urdf_path)
        ...
        self.file_list = self.get_file_list(
            self.root_path, self.splits_data, self.splits, self.categories
        )
    ...
    def __getitem__(self, item):
        ...
        file_path = self.file_list[
            item
        ]  # e.g., "./data/DFCData/datasetv3.1/core/bottle-asdasfja12jaios9012/00000.npz"
        ...
        global_translation = np.array(
            [qpos_dict["WRJTx"], qpos_dict["WRJTy"], qpos_dict["WRJTz"]]
        )  # [3]
        global_rotation_mat = np.array(
            transforms3d.euler.euler2mat(
                qpos_dict["WRJRx"], qpos_dict["WRJRy"], qpos_dict["WRJRz"]
            )
        )  # [3, 3]
        ...
        obj_pc_path = pjoin(
            self.root_path, "DFCData", "meshes", category, instance_no, "pcs_table.npy"
        )
        pose_path = pjoin(
            self.root_path, "DFCData", "meshes", category, instance_no, "poses.npy"
        )
        pcs_table = torch.tensor(
            np.load(obj_pc_path, allow_pickle=True), dtype=torch.float
        )
        pose_matrices = torch.tensor(
            np.load(pose_path, allow_pickle=True), dtype=torch.float
        )
        index = (
            (torch.tensor(plane[:3], dtype=torch.float) - pose_matrices[:, 2, :3])
            .norm(dim=1)
            .argmin()
        )
        ...
        pose_matrix[:2, 3] = 0
        pc = (
            pcs_table[index] @ pose_matrix[:3, :3].T + pose_matrix[:3, 3]
        ) / recorded_data["scale"].item()
        ...
        distances = (
            ...
            * torch.rand(n_samples_table_extra, dtype=torch.float) ** 0.5
        )
        theta = 2 * np.pi * torch.rand(n_samples_table_extra, dtype=torch.float)
        table_pc_extra = torch.stack(
            [
                distances * torch.cos(theta),
                distances * torch.sin(theta),
                torch.zeros_like(distances),
            ],
            dim=1,
        )
        table_pc = torch.cat([table_pc, table_pc_extra])
        table_pc_cropped = table_pc[
            table_pc[:, 0] ** 2 + table_pc[:, 1] ** 2 < max_diameter
        ]
        table_pc_cropped_sampled = pytorch3d.ops.sample_farthest_points(
            table_pc_cropped.unsqueeze(0), K=1000
        )[0][0]
        object_pc = (
            torch.cat([object_pc, table_pc_cropped_sampled])
            - pose_matrix[:3, 3] / recorded_data["scale"].item()
        ) @ pose_matrix[
            :3, :3
        ]  # [N, 3]

        if self.dataset_cfg["fps"]:
            obj_pc = pytorch3d.ops.sample_farthest_points(
                object_pc.unsqueeze(0), K=self.num_obj_points
            )[0][
                0
            ]  # [NO, 3]
        else:
        ...
        if self.cfg["network_type"] == "ipdf":
            ...
            obj_gt_rotation = (
                global_rotation_mat.T
            )  # so that obj_gt_rotation @ obj_pc is what we want
            ...
            gt_hand_mesh = self.hand_builder.get_hand_mesh(
                hand_rotation_mat, hand_translation, qpos=qpos
            )
            gt_hand_pc = (
                sample_points_from_meshes(
                    gt_hand_mesh, num_samples=self.num_hand_points
                )
                .type(torch.float32)
                .squeeze()
            )  # torch.tensor: [NH, 3]
            ...
            if self.dataset_cfg["perturb"]:
                ...
                pert_hand_mesh = self.hand_builder.get_hand_mesh(
                    hand_rotation_mat, pert_hand_translation, qpos=pert_qpos
                )
                pert_hand_pc = (
                    sample_points_from_meshes(
                        pert_hand_mesh, num_samples=self.num_hand_points
                    )
                    .type(torch.float32)
                    .squeeze()
                )  # torch.tensor: [NH, 3]
        ...
        for category in categories:
            for split in splits:
                for instance in splits_data[category][split]:
                    ...
                    files = list(
                        filter(
                            lambda file: file[0] != "." and (file.endswith(".npz")),
                            os.listdir(file_dir_path),
                        )
                    )
                    files = list(map(lambda file: pjoin(file_dir_path, file), files))
                    file_list += files
